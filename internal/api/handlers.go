...
// --- Hermes Nous Portal tool gateway proxy (admin) ---
// These handlers proxy the official Hermes dashboard APIs with a Hermes JWT.
// They never switch HERMES_MODEL_GATEWAY_URL; inference stays on LiteLLM.

func (s *Server) handleListHermesProvidersOAuth(w http.ResponseWriter, r *http.Request) {
	providers, err := s.backend.ListHermesProvidersOAuth(r.Context())
	if err != nil {
		writeError(w, r, err)
		return
	}
	writeList(w, providers)
}

func (s *Server) handleStartHermesNousOAuth(w http.ResponseWriter, r *http.Request) {
	sess, err := s.backend.StartHermesNousOAuth(r.Context())
	if err != nil {
		writeError(w, r, err)
		return
	}
...
func (s *Server) handlePollHermesNousOAuth(w http.ResponseWriter, r *http.Request) {
	sessionID := chi.URLParam(r, "session_id")
	if strings.TrimSpace(sessionID) == "" {
		writeError(w, r, errBadRequest("session_id is required"))
		return
	}
	sess, err := s.backend.PollHermesNousOAuth(r.Context(), sessionID)
	if err != nil {
		writeError(w, r, err)
		return
	}
...
func (s *Server) handleListHermesToolsets(w http.ResponseWriter, r *http.Request) {
	toolsets, err := s.backend.ListHermesToolsets(r.Context())
	if err != nil {
		writeError(w, r, err)
		return
	}
	writeList(w, toolsets)
}

func (s *Server) handleSetHermesToolsetProvider(w http.ResponseWriter, r *http.Request) {
	name := chi.URLParam(r, "name")
	if strings.TrimSpace(name) == "" {
		writeError(w, r, errBadRequest("toolset name is required"))
		return
	}
	var req struct {
	...
	if strings.TrimSpace(req.Provider) == "" {
		writeError(w, r, errBadRequest("provider is required"))
		return
	}
	if err := s.backend.SetHermesToolsetProvider(r.Context(), name, strings.TrimSpace(req.Provider)); err != nil {
		writeError(w, r, err)
		return
	}
	// Return updated toolsets for convenience.
	toolsets, err := s.backend.ListHermesToolsets(r.Context())
	if err != nil {
		// If list fails, just return 204.
		w.WriteHeader(http.StatusNoContent)
		return
	}
	// Find the updated toolset to return.
