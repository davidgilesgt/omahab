...
Browser extension "Save to Omahab" (§15.3); §12 deployment environments, project manifest parser, Omarchy git-sync state machine; §14 untrusted-PR microVM reviewer isolation; §21 desktop-keyring storage, Syncthing device enrollment from clientd, action-picker stubs; generated OpenAPI Go/TS types; RPi5 board profile (TODO defers the RPi image); preflight signed-package/clock-skew hardening (appliance images replace the imperative preflight); release-signing redesign for the flake era (minisign flow retired with the installer; Nix store-path signing to be decided). NixOS-port leftovers: hermes image publication (bundle skipped until ghcr.io/omahab/hermes exists), omahab-once packaging as a flake input (project deploys stay ErrNotConfigured until then).
